...
def detect(opt, *args):
    ...
    for frame_idx, (path, img, im0s, vid_cap) in enumerate(dataset): #for every frame
        ...
        for i, det in enumerate(pred): #for each detection in this frame
            ...
            im0 = draw_roi(im0, x_roi, y_roi, w_roi, h_roi)
            ...
            for x1,y1,x2,y2,conf,detclass in det.cpu().detach().numpy():
                """
                    If object lies in the region of interest of background, we add to list obj tracking
                """
                x_c, y_c = (min(x1, x2) + abs(x1 - x2) / 2), (min(y1, y2) + abs(y1 - y2) / 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str,
                        default='yolov5/best.pt', help='model.pt path')
    # file/folder, 0 for webcam
    parser.add_argument('--source', type=str,
                        default='../dataset/aic22_track4_testA_video/TestA/testA_1.mp4', help='source')
    ...
    parser.add_argument('--sort-min-hits', type=int, default=6,
                        help='start tracking only after n number of objects detected')
    parser.add_argument('--sort-iou-thresh', type=float, default=0.5,
                        help='intersection-over-union threshold between two frames for association')
    
    args = parser.parse_args()
    args.img_size = check_img_size(args.img_size)
    """
        Get region of interest bounding box coordinate for the  video
        video_path  format: ./___/___/___/..../testA_X.mp4      => find X: index of video
    """
